...
public abstract class LuaInjectorAnnotation extends LuaMethodAnnotation implements InjectorChef {
    ...
    protected static MixinMethodBuilder.WriteFactory createInjectWriteFactory(Script script, String eventName) {
        ...
        return (methodVisitor, desc, paramTypes) -> {
            ...
            AsmUtil.getScript(methodVisitor, script);// <- 0
            methodVisitor.visitMethodInsn(INVOKEVIRTUAL, Owners.SCRIPT, "getExecutor", "()Ldev/hugeblank/allium/loader/ScriptExecutor;", false); // <- 0 | -> 0
            methodVisitor.visitMethodInsn(INVOKEVIRTUAL, Owners.SCRIPT_EXECUTOR, "getMixinLib", "()Ldev/hugeblank/allium/loader/lib/MixinLib;", false); // <- 0 | -> 0
            methodVisitor.visitLdcInsn(eventName); // <- 1
            methodVisitor.visitMethodInsn(INVOKEVIRTUAL, Owners.MIXIN_LIB, "getEvent", "(Ljava/lang/String;)Ldev/hugeblank/allium/api/event/MixinMethodHook;", false); // <- 0 | -> 0, 1
